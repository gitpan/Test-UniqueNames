package Test::UniqueNames;

=head1 NAME

Test::UniqueNames - Make sure all of your tests have unique names
...
Test names aren't required by most testing modules, but B<by default Test::UniqueNames counts tests without names as failures>.  You can change that behavior by importing C<unnamed_ok>.
...
sub import {
	$do_end_test = 1;
    Test::UniqueNames::Tracker->unnamed_ok(1) if grep { $_ eq 'unnamed_ok' } @_;
...
wrap 'Test::Builder::ok', post => sub {
    ...
    Test::UniqueNames::Tracker->add_test( $name, $line );
};
...
sub had_unique_test_names {
	...
	my ( $ok, $diag );
    if( @{ Test::UniqueNames::Tracker->failing_tests } > 0 ) {
        $ok = 0;

        my $num_failures = scalar @{ Test::UniqueNames::Tracker->failing_tests };
        $diag = "The following $num_failures test name(s) were not unique:\n"
            . "Test Name                               Occurrences     Line(s)\n"
            . "----------------------------------------------------------------";

        for my $test ( @{ Test::UniqueNames::Tracker->failing_tests } ) {
...
Please report any bugs or feature requests to C<bug-test-uniquenames at rt.cpan.org>, or through
the web interface at L<http://rt.cpan.org/NoAuth/ReportBug.html?Queue=Test-UniqueNames>.  I will be notified, and then you'll
